...
<script>
  import { fromEntries, chunk } from '@/assets/js/form'
  export default {
    props: {
      config: Object,
    },
    components: {
      selectBar: {
        ...
        render(h, {props: {value = '', list = [], callback}, data: {attrs = {}}, listeners: {input}}){
          return h('el-select', {class: 'width-full selectBox', props: {value, ...attrs}, on: {change(v) {input(v); callback(v)}}}, list.map(o => h('el-option', {props: {...o, key: o.value}})))
        }
      },
      checkbox: {
        functional: true,
    ...
    methods: {
      ...
      /*
       * 这里区分了两种情况(footer默认为true，代表默认会展示所封装的form组件的查询、重置按钮)：
       * 1、不使用封装的form组件中的查询、重置按钮，则需要使用回调的方式获取form表单的值
       * 2、使用封装的form组件中的查询、重置按钮，则需要在使用时通过父组件向封装的form子组件传一个submit函数来获取form表单的值
       */
      submit(cb = () => {}) {
        // 第一种情况
